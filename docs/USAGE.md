...
Windows 下，插件发起的 Git 命令自动启用命令级 `core.longpaths=true`，覆盖创建、检出、状态检查以及验收和交付，避免 Worktree 路径前缀变长导致原仓库可检出、隔离目录却失败。不修改全局或仓库 Git 配置，也不自动跳过 LFS；LFS 服务拒绝访问仍需单独处理。

在现有 `dsh-git-worktree` 插件配置中可设置（单位：毫秒）：

| 配置 | 默认值 | 有效范围 |
| --- | --- | --- |
| `gitTimeoutMs` | `120000`（2 分钟） | 1000–3600000 的整数 |
| `worktreeAddTimeoutMs` | `300000`（5 分钟） | 1000–3600000 的整数 |

`worktreeAddTimeoutMs` 是注册与检出的共享创建时限，不是每一步重新计时。普通 Git 查询仍使用 `gitTimeoutMs`；Worktree 删除保持 5 分钟。超时后还需要等待进程树退出及安全检查，因此失败反馈可能晚于配置时限。没有额外的环境变量配置。

创建失败时，插件只在确认进程已退出、目录及 Git 元数据属于本次创建，且残留文件原始内容可验证时清理本次尝试。清理成功会恢复创建前的会话绑定，可以重新点击创建；不会自动重复等待另一个完整创建周期，也不会修改 Local 文件或提交。

以下情况会保留现场并报告需要恢复：进程退出无法确认、路径被替换、未知文件或元数据、`initializing` / `index.lock` 等锁残留、子模块、内容转换规则，或原始内容无法验证（例如转换后的换行符）。稀疏检出保留 Git 原生的文件选择语义，但失败时不保证自动清理。此前版本留下的残留没有本次创建凭据，不会自动删除；新协议的未完成创建也不会被“恢复”或“放弃”误判为可直接接管或删除。

不要通过全局 `git worktree prune`、删除未知锁文件或递归删除目录来试错。出现需要恢复的状态时，保留错误信息及现场，核对残留归属后再处理。
...
重装后先 `dsh plugin --profile web list` 确认版本，再重启 Harness。注意 pnpm 的供应链策略会推迟安装刚发布的版本：新版本发布后几天内，不带版本号的 `add` 可能仍解析到旧版。需要立即安装时，把精确版本加入 profile `pnpm-workspace.yaml` 已有的 `minimumReleaseAgeExclude` 列表（如 `- dsh-git-worktree@0.9.2`）后重新安装。
